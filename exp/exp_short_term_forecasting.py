    ...
    def train(self, setting):
        ...
        for epoch in range(self.args.train_epochs):
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(train_loader):
                ...
                outputs, features = self.model(batch_x, None, dec_inp, None)
    ...
    def vali(self, train_loader, vali_loader, criterion):
        ...
        with torch.no_grad():
            ...
            for i in range(len(id_list) - 1):
                outputs[id_list[i]:id_list[i + 1], :, :], features = self.model(x[id_list[i]:id_list[i + 1]], None,
                                                                      dec_inp[id_list[i]:id_list[i + 1]],
                                                                      None)
                outputs[id_list[i]:id_list[i + 1], :, :] = outputs[id_list[i]:id_list[i + 1], :, :].detach().cpu()
