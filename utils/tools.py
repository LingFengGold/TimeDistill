...
class EarlyStopping:
    ...
    def __call__(self, val_loss, model, path, regressor=None):
        score = -val_loss
        if self.best_score is None:
            self.best_score = score
            self.save_checkpoint(val_loss, model, path, regressor)
    ...
    def save_checkpoint(self, val_loss, model, path, regressor=None):
        if self.verbose:
            print(f'Validation loss decreased ({self.val_loss_min:.6f} --> {val_loss:.6f}).  Saving model ...')
        torch.save(model.state_dict(), path + '/' + 'checkpoint.pth')
        print(f"Save {path}/checkpoint.pth")
        if regressor is not None:
            torch.save(regressor.state_dict(), path + '/' + 'checkpoint_regressor.pth')
